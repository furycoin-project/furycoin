...
#if defined(NDEBUG)
# error "Furycoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0f853d7fb0da74a587de3a5e6b4890ba8e11e5303b2763a48ff8cc2e8a74d3ed");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Furycoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Furycoin Signed Message:\n";
...
{
    // Furycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Furycoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Furycoin: 1 day
static const int64 nTargetSpacing = 5 * 60; // Furycoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Furycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Furycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5d8c8aeacfc1ce1a5d1d39fd95de3392250c0fb4a3fcf4ad31898b3cd9545916");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Furycoin is created!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04051668385add116d4ec169c945b1571c164f4275dde4f5a514b49e36e406004ea30aad90d614fe80de734e6fea6f52deedbc3a4dd8f86481f19342793ca08801") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1518281230;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087546864;

        if (fTestNet)
        {
            block.nTime    = 1518281213;
            block.nNonce   = 386270263;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x79036c3eea7e0385b2fddc34ba57680fd5df556f4304582e3e40719ef9cdc9e4"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc2, 0xba, 0xd2 }; // Furycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FurycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FurycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FurycoinMiner(CWallet *pwallet)
{
    printf("FurycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("furycoin-miner");
    ...
    try { loop {
        ...
        printf("Running FurycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FurycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FurycoinMiner, pwallet));
}
